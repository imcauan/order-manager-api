...
import { UpdateMealDto } from "../meals/dtos/update-meal.dto";
...
export class TablesService {
    ...
    async create(data: CreateTableDto): Promise<void> {
        const tableExists = await this.prismaService.tables.findFirst({ where: { number: data.number }});
    ...
    async getById(id: string): Promise<Tables> {
        try {
          return this.prismaService.tables.findFirst({ where: { id }});
        } catch (error) {
            throw new BadRequestException(error);
        }
    }
    ...
    async update(id, data: UpdateTableDto) {
        const tableExists = await this.prismaService.tables.findUnique({ where: { id }});
